...
const InterestReport = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get('/api/admin-report/interests');
                setData(response.data);
            } catch (err: any) {
                setError(err.response?.data?.message || 'Failed to fetch interests data');
            } finally {
                setLoading(false);
            }
        };
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "amount",
            key: "amount",
            render: (text: string) => <Badge colorScheme="yellow">{text}</Badge>,
        },
        {
            title: "Type",
            dataIndex: "type",
    ...
    return (
        <div className="mt-7">
            <p className="text-3xl font-bold mb-6">Interests</p>
                        ...
                        onClick={() => {
                            try {
                                ...
                                exportToExcel({
                                    data: exportData,
                                    columns,
                                    filename: `interests${dateRange}_${new Date().toISOString().slice(0, 10)}.xlsx`,
                                    sheetName: 'Interests',
                                    title: `Interest Report${dateRange ? ` (${dateRange.replace(/_/g, ' ')})` : ''}`
                                });

                                console.log(`Successfully exported ${exportData.length} interest records`);
